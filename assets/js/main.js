...
(function () {
  ...
  var VIDEOS = [
    {
      youtubeId: "qdHYyRxoMDw",
      titulo: "Águas Profundas",
      capa: "assets/img/video-aguasprofundas.jpg",
    },
    {
      youtubeId: "bovI_Hq5uT8",
      titulo: "Deus não está morto",
      capa: "assets/img/video-deusnaoestamorto.png",
    },
    {
      youtubeId: "PtSBfqbsg10",
      titulo: "Lion And The Lamb",
      capa: "assets/img/video-lionthelamb.jpg",
    },
  ];
  ...
  var FORMSPREE = "https://formspree.io/f/mppwavva";
  ...
  if (grid) {
    ...
    VIDEOS.forEach(function (item) {
      ...
      iframe.title = item.titulo || "Vídeo do YouTube";
      ...
      // proporção padrão do YouTube
      card.style.setProperty("--proporcao", "16 / 9");
  ...
  function validar() {
    ...
    if (
      !marcar(
        c.nome,
        c.nome.value.trim().length >= 2 ? "" : "Informe seu nome."
      )
    )
      erros.push(c.nome);
    if (!marcar(c.sexo, c.sexo.value ? "" : "Selecione uma opção."))
      erros.push(c.sexo);
    ...
    var idadeOk =
      ...
      idade >= 1 &&
      idade <= 120;
    if (!marcar(c.idade, idadeOk ? "" : "Informe uma idade válida."))
      erros.push(c.idade);

    if (
      !marcar(
        c.email,
        RE_EMAIL.test(c.email.value.trim()) ? "" : "Informe um e-mail válido."
      )
    )
      erros.push(c.email);
    ...
    if (!marcar(c.telefone, telOk ? "" : "Use o formato (61) 99999-9999."))
      erros.push(c.telefone);
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      if (FORMSPREE) {
        ...
        fetch(FORMSPREE, {
          ...
          body: JSON.stringify({
            ...
            _subject:
              "Novo interessado por aulas de bateria — " + dados.nomeCompleto,
          }),
        })
          .then(function (r) {
          ...
          .catch(function () {
            if (erroGeral) {
              erroGeral.textContent =
                "Não consegui enviar agora. Tente novamente ou me chame no WhatsApp (61) 99514-9266.";
      ...
      window.location.href =
        ...
        "&body=" +
        encodeURIComponent(corpoMensagem(dados));
